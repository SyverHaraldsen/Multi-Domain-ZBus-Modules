...
#if IS_ENABLED(CONFIG_MDM_BLE_NUS) && IS_ENABLED(CONFIG_MDM_BLE_NUS_RUNNER)

ZBUS_CHAN_DECLARE(BLE_NUS_CHAN);
ZBUS_PROXY_ADD_CHANNEL(uart_proxy, BLE_NUS_CHAN);
#endif

#if IS_ENABLED(CONFIG_MDM_LED) && !IS_ENABLED(CONFIG_MDM_LED_RUNNER)
ZBUS_CHAN_DECLARE(LED_CHAN);
ZBUS_PROXY_ADD_CHANNEL(uart_proxy, LED_CHAN);
#endif

#if IS_ENABLED(CONFIG_MDM_CHANNEL_SOUNDING) && IS_ENABLED(CONFIG_MDM_CHANNEL_SOUNDING_RUNNER)
ZBUS_CHAN_DECLARE(CS_DISTANCE_CHAN);
ZBUS_PROXY_ADD_CHANNEL(uart_proxy, CS_DISTANCE_CHAN);
#endif
#endif
